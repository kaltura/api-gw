...
class KsProcessor extends Helper {
    ...
    crackKs(ks) {
        ...
        let [version, partnerId] = buf.toString('binary').split('|', 3);
        let indexOfSeperator = `${version}|${partnerId}|`.length;
        ...
        if(!this.secrets[partnerId]) {
            throw `Secret not found for partner id [${partnerId}]`;
        }

        let key = this.sha1Raw(this.secrets[partnerId]).slice(0, 16);
        ...
        if (hash !== fieldsHash) {
			throw `Hash [${hash}] doesn't match sha1 on partner [${partnerId}]`;
        }
