...
class AccessLogProcessor extends Helper {
    ...
    constructor(options, server) {
        ...
        let accessLogDir = path.resolve(process.cwd(), path.dirname(options.path.replace(/"/, '')));
        if(!fs.existsSync(accessLogDir)) {
            fs.mkdirSync(accessLogDir);
        }

        let matches;
        let accessLogPath = path.resolve(process.cwd(), options.path);
        if(null !== (matches = /\{([^\}]+)\}/.exec(accessLogPath))) {
            accessLogPath = accessLogPath.replace(matches[0], dateFormat(new Date(), matches[1]));
        }
        this.logger.info('Access log path', accessLogPath);
    ...
    process(data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(this.filtersMatch(request)) {
                ...
                response.on('finish', () => {
                    this.logger.debug(`Request [${request.id}] finished`);
                    ...
                    let request_time = (endDate.getTime() - startDate.getTime()) / 1000;
    ...
    _log(str) {
        fs.write(this.accessLogFile, str + "\n", err => {
            if(err) {
                this.logger.error('Error writing to access log', err);
            }
        });
    }
}
